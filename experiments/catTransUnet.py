...
sys.path.append('/data/users2/washbee/MeshVit/TransUNet') #change this path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n_epochs", default=2, type=int, metavar="N", help="number of total epochs to run",
    )
    ...
    try:
        ...
        assert args.vit_choice < len(CONFIGS_ViT_seg.keys()) # < 8 

        # Convert to a list (optional)
        keys_list = list(CONFIGS_ViT_seg.keys())

        vit_name = keys_list[args.vit_choice]
        ...
        if vit_name.find('R50') != -1:
            config_vit.patches.grid = (int(args.train_subvolumes / args.patch_size), int(args.train_subvolumes / args.patch_size))
        net = ViT_seg(config_vit, img_size=args.train_subvolumes, num_classes=config_vit.n_classes).cuda()
        ...
        customEpochMetricsCallback = CustomEpochMetricsCallback( "macro_dice", 64, args.patch_size, "NA", "NA", "NA", "NA", "NA", modelsize,
        ...
        print("torch.cuda.CudaError caught",e)
        ...
        print("finally")
        if not completed:
            CustomEpochMetricsCallback.log_hyperparams(64, args.patch_size, "NA", "NA","NA", "NA", "NA", "NA", "NA", False, "Not Completed","NA",
                filename = '/data/users2/washbee/MeshVit/experiments/transunet_hsearch_8choice.log',
                logdir="NA")
